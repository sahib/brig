...
func RunCmdline() int {
	...
	app.Commands = []cli.Command{
		{
			Name:     "init",
			Usage:    "Initialize an empty repository and open it",
			Category: repoGroup,
		...
		cli.Command{
			Name:     "open",
			Category: repoGroup,
			Usage:    "Open an encrypted repository. Asks for passphrase",
			Action:   withDaemon(handleOpen, true),
		},
		cli.Command{
			Name:     "close",
			Category: repoGroup,
			Usage:    "Encrypt all metadata in the repository and go offline",
